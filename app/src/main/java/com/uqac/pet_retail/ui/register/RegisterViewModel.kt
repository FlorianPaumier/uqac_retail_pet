...
import com.uqac.pet_retail.data.LoginRepository
import com.uqac.pet_retail.data.Result
...
class RegisterViewModel(private val loginRepository: LoginRepository) : ViewModel() {
    ...
    fun login(username: String, password: String) {
        ...
        if (result is Result.Success) {
            _loginResult.value = RegisterResult(success = RegisteredInUserView(displayName = result.data.displayName))
        } else {
